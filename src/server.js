...
app.post('/api/cv/analyze', upload.single('cv'), async (req, res, next) => {
  try {
    ...
    if (!aiEnabled()) return res.status(400).json({ error: 'Configura AI_PROVIDER y su API key antes de analizar el CV.' });
...
app.post('/api/search', async (req, res) => {
  if (!currentProfile || !currentCvPath || !currentCvText) {
    return res.status(400).json({ error: 'Primero analiza un CV.' });
  }
  ...
  let portals;
  try {
    portals = resolvePortalSelection(requestedPortals);
  } catch (error) {
  ...
  const state = {
    id,
    status: 'queued',
    progress: { phase: 'queued', percent: 0, message: 'Preparando búsqueda...' },
    results: [],
    error: null,
    minMatch
  };
  ...
  (async () => {
    ...
    try {
      ...
      context = await chromium.launchPersistentContext(
        path.resolve(process.env.SEARCH_BROWSER_PROFILE || '.browser-profile-search'),
      ...
      const results = await searchPortals({
        ...
        aiProfile: currentProfile.professional || {},
        minMatch,
        maxPerPortal,
        prefilterScore,
        concurrency,
...
app.post('/api/apply', async (req, res, next) => {
  try {
    ...
    const output = await prepareApplications({
      urls: safeUrls,
      profile: currentProfile,
      cvPath: currentCvPath,
      concurrency: 1
    });
...
app.post('/api/apply-compatible', async (req, res, next) => {
  try {
    if (!currentProfile || !currentCvPath) return res.status(400).json({ error: 'Primero analiza un CV.' });
    const minMatch = Math.max(0, Math.min(100, Number(req.body.minMatch ?? process.env.MIN_MATCH ?? 85)));
    const urls = latestResults
      .filter(x => Number(x.score) >= minMatch && x.recommendation !== 'NO_APLICAR')
      .map(x => x.url);
