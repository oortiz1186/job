function safeJson(text) {
  ...
  catch (error) { throw new Error(`La IA devolvió JSON mal formado: ${error.message}`); }
}
...
function providerLabel(provider) { return ({ deepseek:'DeepSeek', gemini:'Gemini', openai:'OpenAI' })[provider] || provider; }
...
function parseProviderError(provider, status, rawText) {
  ...
  if (status === 402) return `${label} no tiene saldo suficiente (402). Agrega saldo o usa otro proveedor. Detalle: ${message}`;
  if (status === 404) return `${label} no encontró el endpoint o modelo configurado (404). Detalle: ${message}`;
  if (status === 422) return `${label} rechazó parámetros de la solicitud (422). Detalle: ${message}`;
...
async function deepSeekRequest(pathname, options = {}) {
...
export async function testDeepSeek() {
  ...
  const model = String(process.env.DEEPSEEK_MODEL || 'deepseek-v4-flash').trim();
  if (!configured) return { configured:false, ok:false, model, error:'DEEPSEEK_API_KEY no está configurada.' };
  try {
    const response = await deepSeekRequest('/user/balance', { method:'GET' });
    const data = await response.json();
    return {
      configured:true,
      ...
      balances:(data.balance_infos || []).map(x => ({ currency:x.currency, total:x.total_balance }))
    };
  } catch (error) {
    return { configured:true, ok:false, model, error:error.message };
  }
}
...
async function callGemini(prompt) {
  ...
  for (let attempt=1; attempt<=3; attempt++) {
    let response;
    try { response = await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body}); }
    catch (error) {
    ...
    if (response.ok) {
      ...
      if (!text.trim()) throw new Error('Gemini respondió sin contenido utilizable.');
      return text;
    }
...
function configuredProviders(){return Object.keys(PROVIDER_CALLS).filter(p=>String(process.env[PROVIDER_KEYS[p]]||'').trim());}
function requestedProvider(){return String(process.env.AI_PROVIDER||'auto').trim().toLowerCase();}
function fallbackOrder(){
  const configured=configuredProviders();
  const custom=String(process.env.AI_FALLBACK_ORDER||'deepseek,gemini,openai').split(',').map(x=>x.trim().toLowerCase()).filter(x=>PROVIDER_CALLS[x]);
  const preferred=requestedProvider(); const fallback=envFlag('AI_FALLBACK',true);
...
export function aiProviderStatus(){
  const provider=requestedProvider(),configured=configuredProviders(),order=fallbackOrder();
  return {provider,fallback:envFlag('AI_FALLBACK',true),order,providers:{
    deepseek:{configured:configured.includes('deepseek'),model:process.env.DEEPSEEK_MODEL||'deepseek-v4-flash'},
...
export function aiEnabled(){return fallbackOrder().length>0;}
export async function askAI(prompt){
  const order=fallbackOrder();
  if(!order.length)throw new Error('No hay proveedor IA utilizable. Configura una API key y selecciona AI_PROVIDER.');
  const errors=[];
  for(const provider of order){
    try{return await PROVIDER_CALLS[provider](prompt);}
    catch(error){
      errors.push(`${providerLabel(provider)}: ${error.message}`);
      console.warn(`[IA] ${providerLabel(provider)} falló: ${error.message}`);
      if(order.indexOf(provider)<order.length-1)console.warn(`[IA] Probando fallback...`);
    }
  }
